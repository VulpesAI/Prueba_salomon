...
- `AuthProvider` inicializa el estado de sesión, gestiona timers de refresco y se suscribe a `onAuthStateChanged` mediante los helpers modulares al montarse en el navegador. Cada cambio de usuario actualiza `user`/`session` y la suscripción se limpia al desmontar.【F:frontend/context/AuthContext.tsx†L84-L266】【F:frontend/context/AuthContext.tsx†L318-L421】
- El proveedor expone acciones básicas (`login`, `signup`, `loginWithGoogle`, `resetPassword`, `logout`) que resuelven la instancia de autenticación modular antes de cada operación para garantizar que las acciones se ejecuten sólo en cliente.【F:frontend/context/AuthContext.tsx†L423-L507】
- Cualquier hook o componente que dependa de la sesión debe consumir `useAuth()`, que valida que el contexto se utilice dentro del árbol de `AuthProvider` para evitar estados inconsistentes.【F:frontend/context/AuthContext.tsx†L509-L520】

### Inicialización del SDK de Firebase
- `frontend/lib/firebase.ts` valida la configuración pública, reutiliza `initializeApp`/`getApp` para crear una única instancia de Firebase modular y avisa cuando faltan credenciales obligatorias antes de lanzar un error explícito.【F:frontend/lib/firebase.ts†L38-L93】
- Los helpers `getFirebaseAuth`, `getGoogleAuthProvider` y `getFirebaseAnalytics` envuelven `getAuth`, `GoogleAuthProvider`, `signInWithPopup`, `isSupported` y `getAnalytics` para que sólo se ejecuten en el navegador y compartan instancias en memoria.【F:frontend/lib/firebase.ts†L95-L148】

### Variables `NEXT_PUBLIC_*`
- `NEXT_PUBLIC_FIREBASE_*` define las credenciales del proyecto y se combinan con un `fallbackConfig` para ambientes de demo. Todas las claves críticas (`apiKey`, `authDomain`, etc.) deben estar presentes para que `getFirebaseApp` funcione.【F:frontend/lib/firebase.ts†L38-L93】
...
- **Desarrollo local:** define `NEXT_PUBLIC_FIREBASE_*`, `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_CONVERSATION_ENGINE_URL`, `NEXT_PUBLIC_VOICE_GATEWAY_URL` y `NEXT_PUBLIC_FINANCIAL_PROVIDER`. Los hooks proveen valores por defecto (`localhost`) para acelerar la configuración, pero la autenticación sólo funcionará correctamente con credenciales válidas.【F:frontend/lib/firebase.ts†L38-L132】【F:frontend/hooks/useConversationEngine.ts†L77-L206】【F:frontend/hooks/useVoiceGateway.ts†L27-L141】
